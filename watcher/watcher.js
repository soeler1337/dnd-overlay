...
async function syncDefault(gameDir, campaignId, gameId) {
  ...
  if (ambientFile) {
    console.log(`[Watcher] Default Ambient: ${gameId}/default/ambient/${ambientFile}`);
    updates.default_ambient_url = await uploadFile(
      'music',
      sanitizePath(`${campaignId}/default/ambient/${ambientFile}`),
      path.join(defaultDir, 'ambient', ambientFile),
    );
  }
  if (combatFile) {
    console.log(`[Watcher] Default Combat: ${gameId}/default/combat/${combatFile}`);
    updates.default_combat_url = await uploadFile(
      'music',
      sanitizePath(`${campaignId}/default/combat/${combatFile}`),
...
function startWatcher() {
  ...
  for (const gameId of fs.readdirSync(CONTENT_DIR)) {
    ...
    getCampaignId(gameId).then(campaignId => {
      ...
      fs.watch(defaultDir, { recursive: true }, (t, f) => {
        if (!f) return;
        const key = `${gameId}:default`;
        clearTimeout(debounce[key]);
        debounce[key] = setTimeout(async () => {
          await syncDefault(gameDir, campaignId, gameId).catch(e =>
